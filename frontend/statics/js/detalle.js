...
if (id === null) {
    ...
    alert("Esta película no existe. Serás redirigido a la página principal.");
}
...
function buscar_data(data) {
    try {
        ...
        document.getElementById('mainDetalle').innerHTML = '<p>Error al cargar los detalles de la película.</p>';
    }
}
...
function renderizarOpiniones(opiniones) {
    ...
    if (opiniones && opiniones.length > 0) {
        ...
        output = '<div class="contenedorOpiniones"><p>No hay opiniones disponibles.</p></div>';
    }
...
function request_error(error) {
    ...
    document.getElementById('mainDetalle').innerHTML = '<p>No se pudieron cargar los detalles de la película.</p>';
}
